...
func TestStandardBot_PreservesStraight(t *testing.T) {
	...
	if !move.Pass {
		t.Logf("Bot played %+v despite breaking a straight", move.Cards)
		// If it DOES play, it's because our evaluator thinks the penalty for breaking is small.
		// But ideally, a Smart bot passes here.
	} else {
		t.Log("Bot passed to preserve the straight. Good.")
	}
}

func TestStandardBot_FinishesGame(t *testing.T) {
